...
use omamigrate_core::{Decision, Discovery, Operation, Plan, Platform, Proposal, TargetContext};
use std::path::Path;
...
pub fn plan(discovery: &Path, local: bool, out: &Path, yes: bool) -> Result<()> {
    let discovery: Discovery = if local {
        ...
        let s = std::fs::read_to_string(discovery)
            .with_context(|| format!("reading {} (run `omamigrate scan` first, or pass --local)", discovery.display()))?;
    ...
    let mut plan = omamigrate_rules::propose(&discovery, &ctx);
