...
use omam_core::{Discovery, Platform, SourceContext};
use std::collections::BTreeMap;
use std::path::Path;

pub fn list_checks(all: bool) -> Result<()> {
    let platform = Platform::current();
    let checks = if all { omam_checks::all() } else { omam_checks::for_platform(platform) };
...
pub fn discover(only: &[String]) -> Result<Discovery> {
    ...
    for check in omam_checks::for_platform(ctx.platform) {
