...
//!
//! Source machine:  `omam serve`           (advertise, wait for a pair request)
//! Target machine:  `omam migrate <code>`  (discover → propose → migrate)
//!
//! Either machine, offline:
//!   `omam checks`             list what Discover can look at
//!   `omam scan`               run checks here, write discovery.json
//!   `omam plan`               propose from a discovery, decide, write plan.json
//!   `omam apply --dry-run`    show what Migrate would do
...
#[derive(Parser)]
#[command(name = "omam", version, about = "Omarchy migration assistant")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
enum Cmd {
    ...
    Migrate {
        /// Pairing code shown by `omam serve`.
        code: String,
    },
...
fn main() -> Result<()> {
    ...
    match Cli::parse().cmd {
        ...
        Cmd::Serve => anyhow::bail!("pairing transport not implemented yet — use `omam scan` and copy discovery.json"),
        Cmd::Migrate { .. } => anyhow::bail!("pairing transport not implemented yet — use `omam plan --discovery <file>`"),
        Cmd::Usage => {
            let mut buf = Vec::new();
            clap_usage::generate(&mut Cli::command(), "omam", &mut buf);
