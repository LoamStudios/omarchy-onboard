//! Discover on macOS/Linux: scan `~/.ssh`.

use super::{ID, KeyPair, SshConfig};
    ...
    for name in &names {
        ...
        match name.as_str() {
            "config" => {
                ...
                findings.push(
                    Finding::new(ID, Group::Keys, "config", title)
                        .with_value(SshConfig { content, hosts }),
                );
            }
            "known_hosts" => {
                if let Some(fr) = file_ref(&path) {
                    findings.push(
                        Finding::new(ID, Group::Keys, "known_hosts", "~/.ssh/known_hosts")
                            .with_file(fr),
                    );
                }
            }
            _ if name.ends_with(".pub")
            ...
            _ if is_private_key(&path) => {
                ...
                let mut f = Finding::new(
                    ...
                    format!(
                        "SSH key {name} ({}{})",
                        key_type.as_deref().unwrap_or("unknown type"),
                        comment
                            .as_deref()
                            .map(|c| format!(", {c}"))
                            .unwrap_or_default()
